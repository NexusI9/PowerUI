...
export default () => {
    ...
    const buttonPadStyle: ButtonPad = {
        icon: PaintPlus,
        value: 'Create Swatch',
        onClick: (folder: StyleFolder) => dispatch(init({ ...CREATE_SWATCH_CONFIG, folder: folder }))
    };

    const options: FolderOptions = {
        header: {
            add: { icon: SwatchIcon, onClick: (folder: StyleFolder) => dispatch(init({ ...CREATE_SWATCH_CONFIG, folder: folder })) }
        },
        folder: {
            add: { icon: SwatchIcon, onClick: (folder: StyleFolder) => dispatch(init({ ...CREATE_SWATCH_CONFIG, folder: folder })) },
            kebab: [
